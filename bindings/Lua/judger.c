...
/*
assert.h常用于防御式编程。断言（Assertions），
一个断言通常是一个例程（routines）或者一个宏（marcos）。
...
//这里新增加了{"answer_path", LUA_TSTRING, 0},
...
//这里将arg_refs[9]  = &config.answer_path;
//其他依次后移
static int global_judge(lua_State* L) {
    ...
    arg_refs[9]  = &config.answer_path;
    arg_refs[10]  = &config.log_path;
    arg_refs[11] = &config.seccomp_rule_name;
    arg_refs[12] = &config.args;
    arg_refs[13] = &config.env;
    arg_refs[14] = &config.uid;
    arg_refs[15] = &config.gid;
